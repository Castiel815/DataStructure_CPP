...
public:
    ...
    friend std::ostream &operator<<(std::ostream &out, const minHeap<T> &x) {
        x.output(out);
        return out;
    }
...
template<class T>
void minHeap<T>::output(std::ostream &out) const {// Put the array into the stream out.
    copy(heap + 1, heap + heapSize + 1, std::ostream_iterator<T>(std::cout, "  "));
}
